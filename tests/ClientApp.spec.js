...
test.skip("Client app login", async ({ page }) => {
  ...
  const order = (await page.locator("label[class='ng-star-inserted']").textContent());
  console.log(order);
  ...
  for (let i = 0; i < order_counts; ++i) {
    // await page.pause();
    if(order.includes(await page.locator("tr.ng-star-inserted").nth(i).locator("th").textContent())) {
      page.locator("tr.ng-star-inserted").nth(i).locator("button").first().click();
      break;
    }
  }
